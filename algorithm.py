...
def attract_to_connected(path, strength=0.1):
    previous_points = np.roll(path, 1, axis=0)
    next_points = np.roll(path, -1, axis=0)
    attract_forces = (previous_points - path) * strength + (next_points - path) * strength
    return attract_forces


def repulse_from_neighbours(path, strength=0.1, radius=10.0):
    ...
    repulse_forces = np.sum(vectors, axis=1) * strength
    return repulse_forces


def align_to_connected(path, strength=0.1):
    midpoints = (np.roll(path, 1, axis=0) + np.roll(path, -1, axis=0)) / 2.0
    align_forces = (midpoints - path) * strength
    return align_forces


def brownian_perturbate(path, strength=1.0):
    return np.random.randn(*path.shape) * strength
...
def differential_growth(path, attraction_strength=0.1, repulsion_strength=0.1, repulsion_radius=10.0,
    ...
    repulse_forces = repulse_from_neighbours(path, repulsion_strength, repulsion_radius)
